...
class BiResidualLSTM_HAR(BaseLSTM):

    def __init__(self):
        super(BiResidualLSTM_HAR, self).__init__()

        current_path = os.path.abspath(__file__)
        dir = os.path.dirname(current_path)

        stru_file = open(f"{dir}/structure_BiResidualLSTM.json", 'r')
        ...
        self.lstm1 = nn.LSTM(self.n_inputs, int(self.n_hiddens / 2), self.n_layers, bidirectional=True, dropout=0.5)
        self.lstm2 = nn.LSTM(self.n_hiddens, int(self.n_hiddens / 2), self.n_layers, bidirectional=True, dropout=0.5)
