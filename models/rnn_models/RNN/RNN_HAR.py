...
class RNN_HAR(BaseRNN):

    def __init__(self):
        super(RNN_HAR, self).__init__()

        current_path = os.path.abspath(__file__)
        dir = os.path.dirname(current_path)

        stru_file = open(f"{dir}/structure_RNN.json", 'r')
        ...
        self.rnn = nn.RNN(self.n_inputs, self.n_hiddens, self.n_layers, dropout=0.5)
    ...
    def begin_state(self, batch_size, device):
        ...
        return torch.zeros((self.n_layers, batch_size, self.n_hiddens), device=device)