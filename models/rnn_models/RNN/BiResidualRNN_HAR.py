...
class BiResidualRNN_HAR(BaseRNN):

    def __init__(self):
        super(BiResidualRNN_HAR, self).__init__()

        current_path = os.path.abspath(__file__)
        dir = os.path.dirname(current_path)

        stru_file = open(f"{dir}/structure_BiResidualRNN.json", 'r')
        ...
        self.rnn1 = nn.RNN(self.n_inputs, int(self.n_hiddens / 2), self.n_layers, bidirectional=True, dropout=0.5)
        self.rnn2 = nn.RNN(self.n_hiddens, int(self.n_hiddens / 2), self.n_layers, bidirectional=True, dropout=0.5)
