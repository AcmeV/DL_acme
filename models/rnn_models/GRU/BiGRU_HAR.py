...
class BiGRU_HAR(BaseGRU):

    def __init__(self):
        super(BiGRU_HAR, self).__init__()

        current_path = os.path.abspath(__file__)
        dir = os.path.dirname(current_path)

        stru_file = open(f"{dir}/structure_BiGRU.json", 'r')
        ...
        self.gru = nn.GRU(self.n_inputs, int(self.n_hiddens / 2), self.n_layers, bidirectional=True, dropout=0.5)
